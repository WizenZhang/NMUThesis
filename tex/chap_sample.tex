...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.6]{Chapter1.png}
	\caption{第一张插图测试}
...
因此，如果你还想使用subfig宏包而不想使用subcaption宏包，请自己到nmu.cls文件的相关位置更改，具体的使用及修改方法参见相应的宏包说明文档。不过在这里还是不推荐直接去更改nmu.cls文档，除非你对\LaTeX{} 的相关命令很清楚，知道自己在改什么，并且不会对其他格式产生影响。
...
\begin{table}[htb]
	...
	\begin{tabular}{ccc}%l:左对齐；c:居中；r:右对齐
		\toprule
...
\begin{table}[htb]
	...
	\begin{tabu}{lcc}%l:左对齐；c:居中；r:右对齐
		\toprule
		\textbf{编程语言}     & \textbf{优势} & \textbf{劣势} \\
		\midrule
		C$\#$     & 全面集成.Net库  & 跨平台能力太差 \\
...
\begin{longtabu}{cccccc}%l:左对齐；c:居中；r:右对齐
...
\begin{longtabu}{lccccccc} %l:左对齐；c:居中；r:右对齐
...
整个table是由大小一致的单元格组成的，在这个基础上，我们通过合并单元格（行/列）的方式，实现整个table的设计。

\begin{table}[htb]
	\centering  \def\arraystretch{1.5}   %表格间距
	\caption{单元格合并示例表}\label{tab:unit}
	\begin{tabular}{|c|c|c|c|c|c|c|}  %l:左对齐；c:居中；r:右对齐
		\hline		
		\multicolumn{2}{|c|}{ \multirow{2}*{$S_i$} }& \multicolumn{4}{c|}{列合并} &\multirow{2}*{max}\\
		\cline{3-6}
		\multicolumn{2}{|c|}{}&50&100&150&200&\\
		\hline
		\multirow{4}*{\tabincell{c}{行合并\\单元格\\内换行}}&50&0&100&200&300&300\\
		\cline{2-7}
...
Office Visio 是office软件系列中的负责绘制流程图和示意图的软件，是一款便于IT和商务人员就复杂信息、系统和流程进行可视化处理、分析和交流的软件。使用具有专业外观的 Office Visio 图表，可以促进对系统和流程的了解，深入了解复杂信息并利用这些知识做出更好的业务决策。\LaTeX{}中不能直接插入Visio图，需要将Visio图转为pdf格式后再行加入，具体操作步骤如下。
...
\begin{enumerate}[label=\arabic*)]
    ...
    \qquad	（\verb|\includegraphics{nmu-ultimate.pdf}|）；
...
\begin{figure}[htb]
	...
	\begin{subfigure}[b]{.45\textwidth}
		\centering
		\begin{tikzpicture}
		\begin{axis}[width=7cm, height=6.7cm, 
		xlabel={x轴$(t)$}, ylabel={y轴$(m)$},
		xmin=-0.5,xmax=5.5,ymin=0,ymax=6.5,
		xtick={-0.5,1,2,3,4,5},
		ytick={0,1,2,3,4,5,6}]                        % 添加坐标
		...
		\end{axis}                         % 结束坐标
		\end{tikzpicture}	
		\caption{有折线图}\label{subfig:nosmooth}
	\end{subfigure}
	\qquad  
	\begin{subfigure}[b]{.45\textwidth}
		\centering
		...
		\end{axis}
		\end{tikzpicture}
		\caption{光滑折线图}\label{subfig:smooth}
	\end{subfigure}
	\caption{线图}\label{fig:curve}
\end{figure}	
...
%%----------------------------------------------------------------
\subsection{特殊符号\footnote{\href{https://blog.csdn.net/caiandyong/article/details/53351737}{更多参看LaTex常用特殊符号对应表：https://blog.csdn.net/caiandyong/article/details/53351737}}}\label{subsec:sysmbol}
