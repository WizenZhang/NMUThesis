...
\begin{figure}[htb]
	\centering
	\includegraphics[width = 8cm]{Chapter1.png}
	\caption{第一张插图测试}
...
3. 将pdf格式文件复制到figures文件夹；

  \qquad (例如盲审封皮文字： \verb|\includegraphics\{nmu-blind.pdf\}|)；

4. 或另存为eps格式，将eps格式文件复制到figures文件夹，类似插入图片的方式将图插入。

\section{数学环境}

\subsection{特殊符号}

更多参看\href{https://blog.csdn.net/caiandyong/article/details/53351737}{LaTex常用特殊符号对应表：https://blog.csdn.net/caiandyong/article/details/53351737}
\begin{center}
	\begin{tabular}{cc|cc|cc|cc}
		\toprule
		符号  & 命令 &符号 & 命令 &符号 & 命令 &符号 & 命令  \\
		\midrule
		尖号\^{} & \verb|\^{}|  & $\pm$ & \verb|\pm|  & $\bigodot$ & \verb|\bigodot|  & $\neq$ & \verb|\neq| \\
		对号$\surd$  & \verb|\surd|  & $\times$ & \verb|\times|  & $\bigotimes$ & \verb|\bigotimes| & $\approx$ & \verb|\approx|\\
 		大点$\bullet$  & \verb|\bullet| & $\div$ & \verb|\div|  & $\bigoplus$ & \verb|\bigoplus| & $\equiv$ & \verb|\equiv|\\
		小点$\cdot$  & \verb|\cdot|    & $\sum$ & \verb|\sum| & $\ast$ & \verb|\ast| & $\leqslant$ & \verb|\leqslant|\\
		省略$\cdots$  & \verb|\cdots|  & $\prod$ & \verb|\prod| & $\mid$ & \verb|\mid| & $\geqslant$ & \verb|\geqslant|\\
		属于 $\in$ & \verb|\in|    & $\because$ & \verb|\because|   & $\forall$ & \verb|\forall|  & $\alpha$ & \verb|\alpha|\\
		不属于$\notin$ & \verb|\notin| & $\therefore$ & \verb|\therefore| & $\exists$ & \verb|\exists|  & $\beta$ & \verb|\beta|\\
		\bottomrule
	\end{tabular}
\end{center}
