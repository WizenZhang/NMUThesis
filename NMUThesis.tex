...
%%=================================================================
% 设置目录深度（马克思主义学院）
% 当深度为1时，对应着2级标题，即章和节 (chapter1 和 1.1、1.2)；
% 当深度为2时，对应着章、节、小节 (chapter1、1.2、1.2.1)。
\setcounter{tocdepth}{1}
% 最终版英文封面是否启用（数信学院）
\Entitle{off}% 启用: on; 关闭: off<默认>;
% 学校信息是否隐去（盲审版封皮和页眉处）
\HideNMU{off}% 启用: on; 关闭: off<默认>;
%=================================================================
% 论文标题
...
%作者姓名（中文）
\author{学生姓名}
%作者姓名（英文），不显示英文封面时，可不填
\authoren{Name of Student}
% 分类号
\classification{TP277}
% 单位代码
\serialnumber{11407}
% 密级
\secretlevel{公开}
...
% 指导教师（英文），不显示英文封面时，可不填
\supervisoren{Name of Supervisor}
% 申请的学位门类
\applyclass{工学硕士}
% 专业名称（中文）
\major{计算机系统结构}
% 专业名称（英文），不显示英文封面时，可不填
\majoren{Master of Science in Computer System Architecture}
% 研究方向
...
% 提交日期（中文）
\submitdate{2019年5月}
% 提交日期（英文），不显示英文封面时，可不填
\submitdateen{May,2019}
% 毕业届（页眉）
\session{2019}
%%=================================================================
% 摘要-{中文}{英文}
...
% 致谢（盲审版不显示）
\input{tex/chap_acknowledgments}
